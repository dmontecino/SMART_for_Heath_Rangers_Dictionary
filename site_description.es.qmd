# Descripción del Sitio {.unnumbered}
...
site_description_es<-  
  ...
  dplyr::mutate(Definición=
                  c(
                  rep("Selecciones uno o más ítemes que estan presentes en el Incidente. Si hay otras características relevantes en el Incidente que no esten en la lista de opciones y que puedan estar asociadas a enfermedad o muerte, es possible seleccionar la opción 'Otros Hallazgos' y un cuadro de texto aparecerá para describirlos", 
...
# need to find how to change the font size to improve the column width
  ...
  theme_box() %>% 
  width(j = "Opción", width=4.5, unit = "cm") %>% 
